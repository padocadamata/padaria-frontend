...
// Ajuste de demanda reprimida (decisão do usuário, 2026-09-01, migration
// 0031/producao_registros.houve_falta): quando o registro está marcado
// com houve_falta=true, a demanda usada pelo algoritmo para aquela
// ocorrência é a venda real + este número fixo de unidades — NUNCA a
// quantidade produzida, e NUNCA a sugestão final. Esta constante é o
// único lugar que expressa esse "+25"; alterar aqui muda o comportamento
// em toda a sugestão, sem precisar tocar no resto do algoritmo.
export const BONUS_FALTA_UNIDADES = 25;
...
// Demanda ajustada para fins de Planejamento (regra fundamental do
// usuário): parte SEMPRE de quantidade_vendida (nunca de
// quantidade_produzida — produzido não é necessariamente igual a
// vendido). O dado real de venda nunca é alterado por isto; este valor
// existe só como entrada do cálculo abaixo, nunca é persistido nem
// reexibido como "vendido".
function demandaAjustadaVenda(registro) {
  return registro.quantidade_vendida + (registro.houve_falta ? BONUS_FALTA_UNIDADES : 0);
}
...
export function calcularSugestaoProducao({ registros, dataAlvo, turno, diasFechados }) {
  ...
  // Entrada do cálculo estatístico (média ponderada/tendência/clamp,
  // todos abaixo, inalterados): demanda ajustada por falta (venda real +
  // 25 quando houve_falta, nunca produzida) multiplicada pelo ajuste
  // percentual de sobra/escassez já existente. O +25 nunca é somado à
  // sugestão final — ele entra aqui, na entrada, e o resto do algoritmo
  // processa normalmente por cima.
  const valoresAjustados = candidatos.map(
    (r) => Math.max(0, demandaAjustadaVenda(r) * (1 + calcularAjustePercentual(r)))
  );
