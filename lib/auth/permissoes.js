...
export const PERMISSOES = {
  ...
  // Migration 0054 (Folha de Pagamento — Fase 1: Cadastro de
  // Funcionários). Só 3 códigos nesta fase — nada de Escala/Pagamentos/
  // FOPAG/Cartão Ponto ainda. .editar cobre também cargos, dependentes,
  // benefícios, ativar/inativar e vincular/desvincular usuário — nenhum
  // código adicional para essas subáreas (decisão desta rodada, evita
  // permissão que nada usaria ainda). Concedida inicialmente só a
  // proprietario_admin no banco.
  FUNCIONARIOS_VISUALIZAR: 'funcionarios.visualizar',
  FUNCIONARIOS_INSERIR: 'funcionarios.inserir',
  FUNCIONARIOS_EDITAR: 'funcionarios.editar',
};
...
export const MODULOS = {
  ...
  // 'aparencia' (rota /admin-aparencia) foi removida do catálogo em
  // 2026-09-12 -- aparência deixou de ser configurável pela interface
  // (ver lib/branding/tema.js). PERMISSOES.APARENCIA_EDITAR continua
  // existindo (código real, seedado no banco desde a migration 0001) só
  // não tem mais nenhuma tela/rota associada -- não removida do banco
  // nesta rodada por decisão explícita (nenhuma migration de limpeza).
  ...
  // Migration 0054, Fase 1. Rótulo já é o nome da ÁREA final ("Folha de
  // Pagamento"), não só do módulo atual ("Cadastro de Funcionários") --
  // a rota aponta para o único módulo funcional desta fase (/funcionarios),
  // mas o rótulo já reflete a área maior para não precisar renomear o
  // item de menu quando Escala/Pagamentos/FOPAG/Cartão Ponto chegarem
  // (eles crescem como sub-rotas dentro de /funcionarios/*, com uma
  // navegação interna própria — mesmo padrão já usado por Produção,
  // ver components/producao/NavegacaoProducao.js).
  funcionarios: { rota: '/funcionarios', label: '🧑‍🍳 Folha de Pagamento', permissao: PERMISSOES.FUNCIONARIOS_VISUALIZAR },
};
...
// tocada de novo.
export const ITENS_NAVEGACAO_PRINCIPAL = ['dashboard', 'fornecedores', 'producao', 'pedidos', 'catalogo', 'agenda', 'funcionarios'];
